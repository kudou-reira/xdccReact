...
// set options object
  , ircOptions = {
      ...
      , autoRejoin: true
      , autoConnect: true
      , channels: [ '#HorribleSubs', '#Exiled-Destiny', '#UTW', '#mdcx', '#commie-subs', '#doki']
      , secure: true
      , selfSigned: true
      , certExpired: true
      , stripColors: true
...
ircXdcc('irc.rizon.net', user, ircOptions)
  .then(function(instance) {
    ...
    for (var i = 0; i < result.optimizedBots.length; i++) {
      for (var j = 0; j < result.optimizedBots[i].length; j++) {
        ...
        //       }
        //   });
        console.log("this is singlebottouse", singleBot.BotToUse);
        console.log("this is packID", singleBot.PackNumber);
        var singleTask = function() {
          botInstance.xdcc({ botNick: singleBot.BotToUse, packId: singleBot.PackNumber })
          .then(function(xdccInstance) {})
          .catch(function(err) {
              if(err.code) {
                  console.error('Error ' + err.code + ': ' +  err.message);
    ...
    console.log("this is allTasks one deep", allTasks[0]);
    console.log("this is allTasks two deep", allTasks[0][0]);

    // async.parallel(allTasks[0], function(err, results) {
    ...
    for(var k = 0; k < allTasks.length; k++) {
      var singleParallel =
          async.parallel(allTasks[k], function(err, results) {
          console.log("this is the tasks parallel");
          console.log("these are the results in tasks parallel", results);
        });
  ...
  //   var first = function(){
  //       botInstance.xdcc({ botNick: 'Ginpachi-Sensei', packId: '4106'})
  //     .then(function(xdccInstance) {
  //         console.log("this is bot instance", xdccInstance);
  //     })
  //     .catch(function(err) {
  ...
  //    var second = function() {
  //        botInstance.xdcc({ botNick: 'Cerebrate', packId: '2101'})
  //     .then(function(xdccInstance) {
  //         console.log("this is bot instance", xdccInstance);
  //     })
  //     .catch(function(err) {
